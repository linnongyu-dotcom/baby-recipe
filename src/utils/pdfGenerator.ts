...
function buildRecipeHTML(weeklyPlan: WeeklyPlan, ageLabel: string): string {
...
export async function downloadRecipePDF(
  ...
  const htmlContent = buildRecipeHTML(weeklyPlan, ageLabel);
  ...
  try {
    ...
    pdf.save(`宝宝营养食谱_${ageLabel}_${dateStr}.pdf`);
  } finally {
